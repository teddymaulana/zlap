...
export default function OrderCheckoutLink({ order, lineCount }: { order: Order; lineCount: number }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await generateOrderCheckoutLink(order.id);
      } catch (err) {
    ...
    <div className="mb-6 rounded border p-4 text-sm">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2">
        {order.customer_email ? (
          <p className="text-xs text-gray-500">Sends to {order.customer_email}</p>
        ) : (
          <p className="text-xs text-gray-500">Save a customer email above before generating a checkout link.</p>
        )}
        {lineCount === 0 && (
        ...
        <button
          type="submit"
          disabled={isPending || lineCount === 0 || !order.customer_email}
