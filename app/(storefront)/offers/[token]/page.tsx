...
import { getOfferByToken, createOfferOrderAndCharge, type OfferCheckoutInfo } from "@/app/actions/offers";
...
export default function OfferCheckoutPage() {
  ...
  const [offer, setOffer] = useState<OfferCheckoutInfo | null>(null);
  ...
  useEffect(() => {
    getOfferByToken(token).then((res) => {
      if (!res) {
        setLoadError("We couldn't find this offer.");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto w-full max-w-md px-4 py-10">
        <h1 className="mb-6 text-lg font-semibold">Complete your offer</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          >
            <span className={isSubmitting ? "invisible" : ""}>{`Pay ${formatMoney(offer.offeredPrice * offer.qty)}`}</span>
