...
export default function AddressRegionSelect({
  ...
  // A previously-composed value in the exact same format this component
  // emits (see the onChange effect below) — used to restore the cascading
  // selects from a device-local saved address (app/(storefront)/checkout/page.tsx).
  // Resolved once on mount; not a controlled prop.
  ...
  // Resolve a saved "Village, District, City, Province 12345" string back
  // into codes for every level, one lookup per level (each depends on the
  // previous level's resolved code). All the resulting setState calls land
  // in the same effect callback, so React batches them into one render —
  // by the time the RegionCombobox children first mount (keyed on their
  // parent's code), `cities`/`districts`/`villages` already hold the
  // matching options, so each shows its restored name immediately instead
  // of needing a later prop update RegionCombobox isn't designed to react to.
  useEffect(() => {
    if (!initialRegion || provinces.length === 0) return;

    const match = initialRegion.match(/^(.+), (.+), (.+), (.+) (\d{5})$/);
    if (!match) return;
    const [, villageName, districtName, cityName, provinceName, postal] = match;

    let cancelled = false;
    (async () => {
      const province = provinces.find((p) => p.name === provinceName);
      if (!province) return;
      const cityList = await getRegionChildren(province.code);
      const city = cityList.find((c) => c.name === cityName);
      if (!city) return;
      const districtList = await getRegionChildren(city.code);
      const district = districtList.find((d) => d.name === districtName);
      if (!district) return;
      const villageList = await getRegionChildren(district.code);
      const village = villageList.find((v) => v.name === villageName);
      if (!village) return;
      if (cancelled) return;
    ...
    // Runs once provinces are loaded — initialRegion is a one-time seed, not
    // a controlled value, so it's intentionally excluded from deps.
